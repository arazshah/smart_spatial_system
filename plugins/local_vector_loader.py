...
def load_local_vector(path: str, options: dict | None = None) -> dict:
    """
    Canonical production loader contract for local vector loading.
    ...
    for name, func in candidates:
        ...
        for args, kwargs in attempts:
            try:
                ...
                normalized = _normalize_local_vector_contract_output(
                    result,
                    source_path=path,
                )
                normalized["metadata"].setdefault(
                    "loader_contract_adapter",
                    "plugins.local_vector_loader.load_local_vector",
                )
                return normalized
    ...
    raise RuntimeError(
        "local_vector_loader could not satisfy canonical contract. "
        f"Errors: {' | '.join(errors[-8:])}"
    )


def _normalize_local_vector_contract_output(
    value,
    *,
    source_path: str,
) -> dict:
    ...
    if isinstance(value, dict):
        for key in ["vector", "geojson", "payload", "output", "result"]:
            inner = value.get(key)
    ...
    if not isinstance(value, dict):
        raise RuntimeError("Vector loader output must be dict-like.")

    if value.get("type") == "FeatureCollection" and isinstance(value.get("features"), list):
        output = dict(value)
    elif isinstance(value.get("features"), list):
        output = {
            "type": "FeatureCollection",
        ...
        raise RuntimeError(
            "Vector loader output must be FeatureCollection or contain 'features'."
        )
...
def load_local_vector(path: str, options: dict | None = None) -> dict:
    """
    Canonical contract for local vector loading.
    ...
    Transitional behavior:
        1. If the file is JSON/GeoJSON, read it directly.
    ...
    if not file_path.exists():
        raise RuntimeError(f"Vector file does not exist: {path}")

    suffix = file_path.suffix.lower()

    # Deterministic JSON/GeoJSON path for current tests/MVP.
    if suffix in {".json", ".geojson"}:
        try:
            payload = json.loads(file_path.read_text(encoding="utf-8"))
        except Exception as exc:
            raise RuntimeError(f"Could not parse vector JSON file {path}: {exc}") from exc

        return _normalize_local_vector_contract_output(
            payload,
    ...
    raise RuntimeError(
        "local_vector_loader could not load vector with canonical contract. "
