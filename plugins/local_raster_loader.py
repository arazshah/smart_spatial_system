...
# --- Canonical Loader Contract Adapter ---

def load_local_raster(path: str, options: dict | None = None) -> dict:
    """
    Canonical production loader contract for local raster loading.
    ...
    # Try common existing implementations in this module.
    candidates = []

    for name in [
        "_load_local_raster_impl",
        "load_raster",
        "read_raster",
        "load",
        "run",
    ...
    for name, func in candidates:
        ...
        for args, kwargs in attempts:
            try:
                ...
                normalized = _normalize_local_raster_contract_output(
                    result,
                    source_path=path,
                )
                normalized["metadata"].setdefault(
                    "loader_contract_adapter",
                    "plugins.local_raster_loader.load_local_raster",
                )
                return normalized
    ...
    raise RuntimeError(
        "local_raster_loader could not satisfy canonical contract. "
        f"Errors: {' | '.join(errors[-8:])}"
    )


def _normalize_local_raster_contract_output(
    value,
    *,
    source_path: str,
) -> dict:
    ...
    if isinstance(value, dict):
        for key in ["raster", "payload", "output", "result"]:
            inner = value.get(key)
    ...
    if not isinstance(value, dict):
        raise RuntimeError("Raster loader output must be dict-like.")

    data = value.get("data")
    metadata = value.get("metadata") or {}

    if not isinstance(data, list):
        raise RuntimeError("Raster loader output must contain 'data' as list.")
    ...
    output["metadata"].setdefault("source_path", source_path)

    return output

# --- Canonical Loader Contract Wrapper (deterministic) ---
...
def load_local_raster(path: str, options: dict | None = None) -> dict:
    """
    Canonical contract for local raster loading.
    ...
    Transitional behavior:
        1. If the file is JSON, read it directly.
        2. Otherwise try existing internal functions in this module.
    """
    options = options or {}
    file_path = Path(path)

    if not file_path.exists():
        raise RuntimeError(f"Raster file does not exist: {path}")

    suffix = file_path.suffix.lower()

    # Deterministic JSON path for current tests/MVP.
    if suffix in {".json", ".geojson"}:
        try:
            payload = json.loads(file_path.read_text(encoding="utf-8"))
        except Exception as exc:
            raise RuntimeError(f"Could not parse raster JSON file {path}: {exc}") from exc

        return _normalize_local_raster_contract_output(
            payload,
            source_path=str(file_path),
        )

    # Try existing implementation paths for real raster files.
    errors = []
    ...
    for name in [
        ...
        attempts = [
            ((), {"path": str(file_path), "options": options}),
            ((str(file_path), options), {}),
            ((), {"path": str(file_path)}),
            ((str(file_path),), {}),
        ]
    ...
    raise RuntimeError(
        "local_raster_loader could not load raster with canonical contract. "
        f"path={path!r}, suffix={suffix!r}, errors={' | '.join(errors[-8:])}"
    )
